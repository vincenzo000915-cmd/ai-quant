...
- AI 经理只控 TP 的 R 位置,**控不了分批比例**(tp1_frac/tp2_frac 写死)→"吃头中段不贪尾"只控了一半。
- **资金费(funding)没进 EV** 回测(只在感知里给经理看)。
- **相关性/方向集中度**没进资金闸(可能开多个高相关同向仓,各自不超额但合起来集中)。见 `feedback_crypto_structural_correlation`。
