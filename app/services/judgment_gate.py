...
def evaluate_exit(side: str, base_candles: list,
    ...
    🚨 P2 硬门关键发现 (与 14k-158 B0 完全一致): 利润锁与 **flat_pct 小固定 TP 策略结构性不兼容** —
       1h flat TP=0.333%价/4h=0.533%价 都 < activate_pct(0.8%价) → 价到 TP 先被平, 利润锁永不触发
       (37天×7策略硬门实测 0 锁利). 利润锁是**趋势哲学**, 只对 sl_mode='atr'/大TP/无TP 的趋势策略
       有用武之地 (即 14k-158 当前休眠的 atr 仓). → P3 接 live 时 **exit_gate 只挂 atr 策略, 别强加给
       flat 策略** (同 14k-158: 高 R:R 是趋势哲学不强加). 当前无 atr 策略 → exit 利润锁随之休眠。
