...
if 0.5 < ratio < 2.0:  OK
if ratio < 0.5 or ratio > 2.0:  REJECT 「数字不一致」
```

LLM 必须真心算（或者照公式套）— 不能凭感觉给 `expected_oos_trades`。
...
def _validate_self_estimate(est: dict) -> tuple[bool, str, dict]:
    """Phase 12.46.3: 强制频率校准 — Python 用 LLM 给的 pct 算 expected_trades，
    跟 LLM 写的 expected_oos_trades 对比。差 > 2x 直接 reject。
