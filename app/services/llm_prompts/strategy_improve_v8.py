...
GENERATE_PHASE_TIMEOUT = 1500   # 25min — v8.3 prompt + 复杂 JSON 输出实测要更多时间
...
"risk_params": {{
  ...
  "order_type": "market",       // Phase 13: 'market' (taker 0.05%) | 'maker' (post_only 0.02%) | 'maker_with_fallback' (0.025% blend)
  "reasoning": "mean-rev 边际利润小，5x 杠杆 + 宽 SL 6% 避免 whipsaw 一笔损 50%"
}}
```

**order_type 选择**:
- `market` (默认): 立即成交，taker fee 0.05% — breakout / 紧追型
- `maker`: post_only 限价，60s 超时 cancel — mean-rev / 不急的 entry
- `maker_with_fallback`: maker 优先，超时 fallback taker — 短 TF 想 maker fee 但又要保证成交

**关键**: 短 TF (15m/30m) 使用 maker 会显著降 fee，原本不可行的策略变可行

**Risk params 推荐准则**（v8.3 — 基于真实数据翻转）：

⚠ 实测警告（AVAX donchian_1d 同策略不同 lev/SL）:
  ❌ 15x lev + 5% SL  → OOS Sharpe -2.6 / PF 0.71（whipsaw 打死）
  ✅ 1x lev + 20% SL → OOS Sharpe **6.63** / PF **2.74**（策略 breathe）

→ **默认低杠杆 + 宽 SL**，高杠杆要有具体理由

| Strategy × TF | leverage | SL% | TP% | 备注 |
|---|---|---|---|---|
| 1d trend/breakout | 1-3x | 15-25 | 25-50 | 数据证 1x+20% Sharpe 6.63 |
| 4h trend/breakout | 2-5x | 8-15 | 15-25 | 4h 主战场 |
| 4h mean-rev | 3-5x | 8-12 | 12-20 | 留 buffer 防假反弹 |
| 1h trend/breakout | 3-7x | 5-10 | 10-15 | 边缘 |
| 1h mean-rev | 3-5x | 5-10 | 8-15 | 边缘 |
| 30m/15m | 2-3x | 3-5 | 5-10 | 高频小 SL，必须 PF≥1.8 |

**绝不 > 10x leverage 除非 PF > 2.5 + 数学证明可行**

高频策略（>60 trades/90d）杠杆 ≤ 3x — 单笔波动小，杠杆放大 SL 概率而非收益
...
# 💎 真正 quant 价值 (v8.3 — 产品定调)

不是追：胜率 70%、低频大波段
要追：**PF 2-3，每笔微利，trades 海量** (Renaissance / Citadel 模式)

数学示例:
  300 trades × avg_win 0.4% (扣 fee 0.2%) × WR 40% + PF=2 =
    wins: 120 × 0.4% = +48% gross
    losses: 180 × 0.4% × (1/2) = -36% gross (PF=2 意味赢/亏 ratio 2:1)
    net: +12% / 90d ≈ AR 50%/年 ✅

→ **高频 + 低 WR + 高 win/loss ratio = 真量化**
→ 写策略时优先思考：能不能 PF=2 然后 trade 400+/90d？
→ WR 不重要，PF 重要

# 🚪 自测门槛（per TF — v8.3 加 win/loss ratio + 短 TF 提高 PF 门槛）

| TF | OOS Sharpe | PF | Trades/90d | avg_win/loss | AR/年 |
|---|---|---|---|---|---|
| 15m | ≥ 1.5 | **≥ 2.0** | ≥ 60 | ≥ 2.5x | ≥ 8% |
| 30m | ≥ 1.5 | **≥ 1.8** | ≥ 50 | ≥ 2.2x | ≥ 8% |
| 1h | ≥ 1.5 | ≥ 1.5 | ≥ 40 | ≥ 1.8x | ≥ 7% |
| 4h | ≥ 1.5 | ≥ 1.4 | ≥ 30 | ≥ 1.6x | ≥ 7% |
| 1d | ≥ 1.5 | ≥ 1.3 | ≥ 12 | ≥ 1.5x | ≥ 5% |

为什么短 TF PF 门槛高？
  - fee 0.2%/笔 × 高频 → 短 TF fee 占比 25-33%
  - 必须 PF ≥ 2 才能扣 fee 后净正
  - **未来 Phase 13 加 maker order 后此门槛会大降**

外加：`decay_pct ≤ 70%`，**SL hit_pct ≤ 60%**

# ⚖️ 短 TF 不被禁，但门槛高 — 写出来就是 quant 圣杯

如果你能写 15m PF=2.0 / 60 trades 策略 → **优先 promote**
如果写不出，回到 4h/1d 找稳妥 — 都 OK

# 🚨 频率估算（v8.2 — 结构化 schema，Python 验证）

**LLM 历史问题：自由写 `expected_oos_trades` 数字 → 系统性 3-9x 高估**
**v8.2 修法：把估算拆成 Python 能验证的字段**

LLM 只负责从 symbol_stats 找数字 + 列 filter 通过率（这是 LLM 擅长的）。
Python 自动算 `expected_trades = primary_pct × filter_pcts × candles / 1.5`。
**如果 LLM 写的 expected_oos_trades 与 Python 算出 > 2x 偏差 → reject before backtest**。
...
- 0 层 (纯 primary trigger): pass_pcts = []，频率 = primary × candles / 1.5
- 1 层: pass_pcts = [x]，频率 = primary × x/100 × candles / 1.5
- 2 层: pass_pcts = [x, y]，频率 = primary × x/100 × y/100 × candles / 1.5
- **不允许 >2 层**

## symbol_stats 哪里找 primary_trigger_pct

| Trigger logic | symbol_stats field |
|---|---|
| RSI<30 (oversold mean-rev) | `rsi14.pct_below_30` |
| RSI>70 | `rsi14.pct_above_70` |
| Stoch K<20 | `stoch14.pct_oversold_below20` |
| Stoch K>80 | `stoch14.pct_overbought_above80` |
| MACD bull cross | `macd_12_26_9.crosses_per_100bars` |
| ADX>25 (trending) | `adx14.pct_above_25` |
| BB squeeze (width<p10) | 估约 10%（p10 的定义）|
| EMA50/200 cross | `trend.flips_per_100bars` |

## Python 校验规则（v8.3 — 放宽 ratio 容差）

```
computed = primary/100 × Π(filter/100) × candles / 1.5
ratio = declared / computed
if 0.3 < ratio < 3.0:  OK    # v8.3 放宽 (filter 正相关 real-world)
if ratio < 0.3 or ratio > 3.0:  REJECT 「数字明显不一致」
```

LLM 必须真心算 — 不能凭感觉给 `expected_oos_trades`。

## 频率估算 — 接受现实

v8.1/v8.2 实测：primary × filter 乘积模型**系统性 3-9x 高估**（filter 正相关）
**修正**：
  1. 多 filter 时主动 ×0.5 buffer (你自己 conservatively 估算)
  2. 单层 filter 通过率上限 50%（不要写 70%+ 除非有数据）
  3. 估算后再 ×0.5 是常态而非例外
...
def _format_iteration_history_v8(history: list[dict]) -> str:
    """v8.1: 更细 — 加 trade_patterns + 频率误差比，让 LLM 看 SL hit% / filter 过严"""
    if not history:
        return '(第一轮)'
    lines = []
    for h in history:
        ...
        for a in h['attempts']:
            ...
            # v8.1: 频率误差比诊断
            est_tr = est.get('expected_oos_trades')
            act_tr = mt.get('oos_trades')
            if est_tr and act_tr and est_tr > 0:
                ratio = act_tr / est_tr
                if ratio < 0.5:
                    lines.append(
                        f'    🚨 频率严重低估: 实际 {act_tr} 是估的 {round(ratio*100)}% '
                        f'(差 {round(1/ratio, 1)}x) → **下轮必须减 1 层 filter 或放宽阈值**，不准只调 risk_params'
                    )
                elif ratio > 1.5:
                    lines.append(
                        f'    ⚠ 频率超估: 实际 {act_tr} 是估的 {round(ratio*100)}% → 下轮加 filter 收紧'
                    )
    ...
    lines.append('')
    lines.append('→ 分析根因：frequency 严重低估 → 减 filter / SL hit% 高 → 改 leverage 配比 / W/L ratio < 1 → 改 TP_pct 或换 logic')
    lines.append('→ **不要**重复同样 logic + params 组合')
    return '\n'.join(lines)


def _validate_self_estimate(est: dict) -> tuple[bool, str, dict]:
    """Phase 12.46.3 / v8.3: 强制频率校准 — Python 用 LLM 给的 pct 算 expected_trades，
    跟 LLM 写的 expected_oos_trades 对比。差 > 3x 直接 reject (v8.3 放宽 2→3 容 filter 正相关)。

    返回 (passed, reason, validation_detail)
    """
    if not est:
        return False, 'self_estimate 缺失', {}
    primary = est.get('primary_trigger_pct')
    filter_pcts = est.get('filter_pass_pcts') or []
    candles = est.get('candles_in_window')
    declared = est.get('expected_oos_trades')

    if primary is None or candles is None or declared is None:
        missing = [k for k in ('primary_trigger_pct', 'candles_in_window', 'expected_oos_trades') if est.get(k) is None]
        return False, f'缺字段: {missing}', {}

    if not isinstance(filter_pcts, list):
        return False, 'filter_pass_pcts 必须是 list', {}
    if len(filter_pcts) > 2:
        return False, f'filter 层数 {len(filter_pcts)} > 2 (上限 2)', {}
    ...
    try:
        ...
        d = float(declared)
        product = p
        for f in filter_pcts:
            product *= float(f) / 100
        computed = product * c / 1.5    # /1.5 bias buffer
    except Exception as e:
        return False, f'数值解析: {e}', {}

    if computed < 0.5:
        return False, f'computed_trades {computed:.1f} < 0.5 (primary/filter 太严)', {
            'computed': round(computed, 2), 'declared': d,
        }

    ratio = d / computed if computed > 0 else 999
    detail = {
        'primary_pct': primary, 'filter_pcts': filter_pcts,
        'candles': candles, 'computed': round(computed, 1),
        'declared': d, 'ratio': round(ratio, 2),
    }
    if ratio < 0.3 or ratio > 3.0:
        return False, f'declared={d} vs computed={computed:.1f} (ratio={ratio:.2f}) — LLM 数学错', detail
    return True, f'computed={computed:.1f} declared={d} ratio={ratio:.2f}', detail
...
def improve_strategies_v8(user_id: int, *,
    ...
    for iteration in range(max_iterations):
        ...
        llm = call_llm(
            ...
            max_tokens=6000,    # v8.3 实测：减小 output 让 sonnet 更快收敛 (10000 → 6000)
            allowed_tools=None,
        ...
        for cand in spec['candidates'][:remaining]:
            ...
            # Phase 12.46.3: 先验 self_estimate 数学一致性 — 不一致直接 reject 不浪费 backtest
            est_passed, est_reason, est_detail = _validate_self_estimate(cand.get('self_estimate'))
            if not est_passed:
                iter_result['attempts'].append({
                    'candidate_type': ctype, 'symbol': symbol, 'timeframe': tf, 'category': cat,
                    'passed': False, 'reason': f'self_estimate 校验 fail: {est_reason}',
                    'metrics': None, 'estimate': cand.get('self_estimate'),
                    'estimate_validation': est_detail,
                })
                rejected.append(iter_result['attempts'][-1])
                continue
