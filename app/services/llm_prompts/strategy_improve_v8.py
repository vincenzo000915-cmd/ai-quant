...
**LLM 历史上系统性 3-9x 高估 trade 次数**:
  - 估 30 → 实际 10 (3x off)
  - 估 45 → 实际 7 (6x off)
  - 估 90 → 实际 10 (9x off)

根因：filter 层数太多 + 没用 symbol_stats 真实数据校准。**导致 OOS Sharpe 即使是 9.86，trades=7 也过不了 30 trades 门槛**。

## 必须遵守

### 1. Filter 层数 ≤ 2 层（不算 min_bars guard）

- 3 层 filter 串联 → 触发率 5-15% × candles，**几乎必定** trade<min
- 2 层 → 触发率 15-30%（合理范围）
- 1 层 → 触发率 30-50%（适合 mean-rev 高频）

### 2. 频率估算必须用 symbol_stats 真实数据

例（AVAX 4h，假设需要 ≥30 trades/90d）:
  - 4h 90 天 candles = 540 根
  - 30 / 540 = 5.6% 触发率门槛
  - **必须** 选 primary trigger pct ≥ 10% 的条件（留 buffer）
  - symbol_stats 给的 `pct_below_30` 就是 RSI<30 实际占比；不要拍脑袋

公式：`expected_trades = primary_trigger_pct × filter_A_pass_pct × filter_B_pass_pct × candles_in_90d / 1.5`
（最后 **/1.5 是 LLM bias 校正**）

### 3. `self_estimate.reasoning_for_estimate` 必须列计算步骤

错误样例（LLM 常犯）：
> "用 RSI<30 + ADX<25 + EMA50>200，月 5 trades"

正确样例：
> "AVAX 4h RSI<30 实际 6.3% × ADX<25 实际 55% = 3.5% × 540 candles = 19 raw → /1.5 buffer = 12 trades/90d"
> "12 < 30 不达标 → 改 RSI<35 (扩大到 13.5%) 或去掉 ADX filter"

### 4. Iter > 0 时强制根据上轮误差校准

如果上轮 `trades_actual / trades_estimated < 0.5`：
  - **必须**减一层 filter，OR
  - **必须**放宽 primary trigger 阈值 (e.g. RSI<30 → RSI<35)
  - 不允许只改 risk_params 不改 trigger 频率
