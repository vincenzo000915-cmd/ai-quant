...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      '✓ 全部内置策略库 + 实时信号循环',
      '📡 <b>信号预告 + 手动执行</b> — 看到策略即将触发，用你设的参数一键手动下单',
      '✓ LIVE 实盘模式（接 OKX 或 Hyperliquid · 单交易所）',
      '✓ 策略库浏览 + 一键上架 + 自动回测筛选',
  ...
  },
  {
    ...
    price: 125,
    badge: '最受欢迎',
    description: '守门员自动盯盘下单 · 你 BYO key 加 AI 经理',
    features: [
      '✓ 包含 Basic 全部功能',
      '🛡️ <b>守门员自动盯盘下单</b> — 实时扫描行情，信号触发自动择时下单（交易所原生止损 + 分批止盈，服务端执行）',
      '🎯 <b>设月度目标</b> — 系统按你的目标自动定难度、控杠杆、盯进度',
      '🧠 <b>AI 经理逐单择参</b> — 结合行情临场判断每一单的杠杆 / 止损 / 止盈（用你自己的 key，没绑也照跑机械托管）',
      '🔑 <b>BYO LLM key</b>（Anthropic / OpenAI / Gemini）— AI 用你自己的账号跑，<b>我们不抽 token 费</b>',
      '✨ AI 工具全开：策略解释 / 自然语言生成 / Regime 解读 / 周复盘 / 个性建议 / 仓位推荐',
      '── 单交易所绑定 (OKX OR Hyperliquid)',
      '✗ 全自动 AI 自动托管 + 多交易所 (需 Team)',
    ],
    cta: '选择 Pro',
    accent: true,
  },
  {
    ...
    features: [
      ...
      '⚡ AI 自动跨所路由 — 信号触发按 EV/手续费选最优交易所执行',
      '🔁 某所余额不足自动顺延下一个交易所 (不漏单)',
      '📊 后台多账户汇总余额视图 + 跨所资金统一调度',
    ],
    cta: '选择 Team',
    accent: false,
  },
...
const FAQ = [
  ...
  { q: '注册免费可以看到什么？', a: '完整 UI（Dashboard / 策略 / 候选池 / Trades / Settings），demo 数据 + 系统展示。所有「动作」按钮禁用。订阅 Basic 解锁信号预告 + 手动执行，Pro 解锁守门员自动盯盘下单 + 设月目标 + 全套 AI 工具（你 BYO key），Team 再加 AI 全自动托管 + 多交易所。' },
  { q: 'Pro 和 Team 有什么本质区别？', a: 'Pro = 守门员替你自动盯盘下单 —— 你设好参数/月目标，它实时扫描行情、信号触发自动择时进出场（AI 经理逐单择参用你自己的 LLM key；没绑也照跑机械托管）。但资金和回撤还是你自己盯。Team = 在 Pro 之上再加「AI 全自动托管」：你只设个目标（例 +20%/30 天），AI 自动跟踪进度 / 回撤保护 / 策略轮换 / 资金跨档扩张 / 同时跑 OKX + Hyperliquid 多账户，适合不想花时间盯盘的人。' },
